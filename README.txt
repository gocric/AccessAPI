 ...
 4. The script has been dockerized using the "Dockerfile" file, that does the following steps:
	a. This line is used to specify the docker OS and we have used Python:3 as the OS, since the program is going to run on python.
	b. The next line mentions the file that needs to be added to the docker image.
	c. Since we are using the "requests" library, we have it preinstalled in the next line.
